...
include::{asciidoc-dir}/configuration/chap-kogito-configuring.adoc[tags=proc-postgresql-persistence-enabling]
...
[role="_abstract"]
For security, {PRODUCT} Management Console relies on Keycloak as authentication service. Once the user is authenticated, Management console will use bearer tokens to communicate with other Kogito services.These tokens are issued by OpenID Connect and OAuth 2.0 compliant authorization servers such as https://www.keycloak.org/about.html[Keycloak].
...
* An instance of Keycloak server running.
...
The {PRODUCT} Management Console, by default has security enabled.
--
...
+
* `kogito.consoles.keycloak.config.url`: The base URL of the keycloak server, such as `https://localhost:8280/auth`. All other keycloak server page and service URLs are derived from this URL.
* `kogito.consoles.keycloak.config.client-id`: The client ID of the application. Each application has a client ID that is used to identify the application.
* `kogito.consoles.keycloak.config.realm`: The realm which has all the sercuity configurations.
* `kogito.consoles.keycloak.config.health-check-url`: This is the health check URL, which is used to check if the keycloak server is properly started.
* `kogito.consoles.keycloak.config.disable-health-check`: It is up to the user to decide whether the health check is required. By default, the health check is always enabled.
* `kogito.consoles.keycloak.config.update-token-validity`: This is the minimum validity in seconds. If the token expires within this time, the token is refreshed.
...
The {PRODUCT} Task Console, by default has security enabled.
--
